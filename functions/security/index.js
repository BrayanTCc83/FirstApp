...
const validateDataType = ( value, type ) => {
    ...
    const recommendation = "modify the type of your data to "+type
    switch ( type ){
        case 'string' : 
        case 'number' :
            return typeof value === type ? true : ErrorHandler.errorHandler( error, recommendation )
        case 'Array<string>' : 
            return Array.isArray( value ) && value.every((valueOn)=>typeof valueOn === 'string') ? true : ErrorHandler.errorHandler( error, recommendation )
    }
}
export {
    encryptPassword,
